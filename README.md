...
| [bandaid](https://github.com/F0Rextasy/bandaid) | `npx skills add F0Rextasy/bandaid` | Catches symptom-suppression patches in a diff — swallowed errors, removed guards, disabled tests, silenced checkers — and forces root-cause fixes. Reproduce → isolate → fix at the cause → gate the diff with the bundled scanner. |
| [aitell](https://github.com/F0Rextasy/aitell) | `npx skills add F0Rextasy/aitell` | Deterministic AI-tell prose detector: 11 rules, published confusion matrix, CI fails if accuracy drops below the published number. Fully offline, stdlib only. |
| [cigate](https://github.com/F0Rextasy/cigate) | `npx skills add F0Rextasy/cigate` | GitHub Actions waste gate: unpinned actions, missing path filters, duplicate jobs, minute budget from a local `gh api` capture. |
| [ci-triage](https://github.com/F0Rextasy/ci-triage) | `npx skills add F0Rextasy/ci-triage` | Classifies a red CI run as regression, flaky, or infra from the log alone, with evidence-linked exit codes. |
| [compressproof](https://github.com/F0Rextasy/compressproof) | `npx skills add F0Rextasy/compressproof` | Reversible context compression proven in CI: SHA-256 byte-exact round-trip plus N/N needle-question answer oracle against a full-text baseline. |
| [docproof](https://github.com/F0Rextasy/docproof) | `npx skills add F0Rextasy/docproof` | Parses every fenced code block in your docs with per-language checkers so examples never rot. |
| [dsh-gate](https://github.com/F0Rextasy/dsh-gate) | `npx skills add F0Rextasy/dsh-gate` | Shell gate that blocks dangerous command shapes before they run. |
| [preflight](https://github.com/F0Rextasy/preflight) | `npx skills add F0Rextasy/preflight` | Pre-push gate: runs the checks that matter before you waste a CI minute. |
| [prove-it](https://github.com/F0Rextasy/prove-it) | `npx skills add F0Rextasy/prove-it` | Turns a claim in prose into a runnable proof harness with pass/fail exit codes. |
| [route-drift](https://github.com/F0Rextasy/route-drift) | `npx skills add F0Rextasy/route-drift` | OpenAPI spec vs code routes drift gate: missing implementation, undocumented route, method mismatch, each with file:line evidence. |
| [sessionaudit](https://github.com/F0Rextasy/sessionaudit) | `npx skills add F0Rextasy/sessionaudit` | Session policy gate: audits agent sessions against committed rules. |
| [shipcheck](https://github.com/F0Rextasy/shipcheck) | `npx skills add F0Rextasy/shipcheck` | Release checklist gate: nothing ships unchecked. |
| [testgate](https://github.com/F0Rextasy/testgate) | `npx skills add F0Rextasy/testgate` | Test-selection gate: runs the tests that can actually fail for a change, skips the rest. |
| [uigate](https://github.com/F0Rextasy/uigate) | `npx skills add F0Rextasy/uigate` | Measurable design-slop lint: WCAG contrast math on literal colors plus disclosed AI-template tells (gradient, card grid, boilerplate). |
| [wincompat](https://github.com/F0Rextasy/wincompat) | `npx skills add F0Rextasy/wincompat` | Windows compatibility gate for cross-platform scripts and CLIs. |
